...
def extract_frames(session: VideoSession, output: Path, sample_fps: float, max_frames: int, jpeg_quality: int) -> dict[str, Any]:
    ...
    return {
        ...
        "sampling": {"requested_fps": sample_fps, "source_frame_stride": sample_every, "jpeg_quality": jpeg_quality},
        "frames": records,
    }
        ...
        records.append(
            {
                ...
                "detector_observation": {
                    ...
                    "heuristic_occupied_chair_candidates": proposed_occupied,
                    "heuristic_occupied_count": len(proposed_occupied),
                },
...
def main() -> int:
    ...
    if args.dry_run:
        return 0
    for session in args.sessions:
        prepared_dir = args.data_dir / "prepared" / session.session_id
        prelabels_dir = args.data_dir / "prelabels" / session.session_id
        review_dir = args.data_dir / "reviews" / session.session_id
        require_new_directory(prepared_dir)
        try:
            manifest = extract_frames(session, prepared_dir, args.sample_fps, args.max_frames, args.jpeg_quality)
            ...
            records = create_review_records(
                prelabels_dir / "predictions.jsonl", args.expected_seat_count, session.expected_occupied_count
            )
